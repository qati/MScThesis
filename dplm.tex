...
\caption{Mag-glória modell szemléltetése.}
\label{fig:ch1}
\end{figure}
...
\caption{Keskeny magból és széles glóriából álló forráshoz tartozó korrelációs függvény. A glória keskeny csúcsot eredményez a korrelációs függvényben, amely a detektor véges felbontásának következtében nem látható.}
\label{fig:ch2}
\end{figure}
...
\begin{equation}
K(\bm{k}_1,\dots,\bm{k}_n)=
\frac{
\int \prod_{i=1}^n\mathcal{S}(\bm{k}_i, \bm{r}_i)
\abs{\Psi^{0}_{\bm{k_1},\dots,\bm{k_n}}(\bm{r_1},\dots,\bm{r_n})}\prod_{i=1}^n d^3 \bm{r}_i
}{\int \prod_{i=1}^n\mathcal{S}(\bm{k}_i, \bm{r}_i)
\abs{\Psi^\mathcal{C}\bm{k_1},\dots,\bm{k_n}}(\bm{r_1},\dots,\bm{r_n})\prod_{i=1}^n d^3 \bm{r}_i},
\end{equation}
...
\end{equation}
ahol $\mu=\frac{m}{2}$ a redukált tömeg, $V_\mathcal{C}$ pedig a Coulomb potenciál.
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.45]{pic/Coulomb/C2_dR_a19.pdf}
\end{figure}
